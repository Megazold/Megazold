*** *** # Hi there, I'm Hermawan 👋

## 🚀 About Me
Software Engineer & Developer at **KPU Kepulauan Seribu**. I build E-Office systems, automation tools, and digital solutions to make bureaucratic processes efficient and transparent.

🔭 Currently working on **Satuga** — an E-Office platform for work assignments and document management
🌱 Learning **AI/ML integration** and **system architecture**
💡 Passionate about **automation**, **digital transformation**, and **open source**

## 🛠️ Tech Stack
**Languages:** Python, JavaScript, TypeScript, PHP, SQL
**Frameworks:** FastAPI, Next.js, Laravel, React
**Tools:** Docker, Git, PostgreSQL, Redis, Nginx
**Platforms:** Notion API, Google Workspace API, Telegram Bot API
**DevOps:** Linux (Ubuntu/Mint), Docker Compose, CI/CD, Cloudflare

## 📊 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Megazold&show_icons=true&theme=dark)

## 📫 Let's Connect
- 📧 Email: hermawan.indar99@gmail.com
- 💼 LinkedIn: [Hermawan Indaryanto](https://linkedin.com/in/hermawan-indaryanto)
- 🏢 KPU Kepulauan Seribu

---

*"Building digital bridges for better governance."*
